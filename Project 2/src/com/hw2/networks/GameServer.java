...
class ClientRequestHandler implements Runnable {
	...
	public ClientRequestHandler(Socket socket, ConcurrentMap<UUID, GameObject> scene) {
...
class ClientConnectionHandler implements Runnable {
	...
	public ClientConnectionHandler(ConcurrentMap<UUID, GameObject> scene) {
		// TODO Auto-generated constructor stub
		this.scene = scene;
	}
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				ClientRequestHandler requestHandler = new ClientRequestHandler(socket, scene);
...
public class GameServer  {
	...
	Adding Platforms
	*/
	public static void createScene(ConcurrentMap<UUID, GameObject>scene) {
		...
		for (int i = 0; i < noOfPlatforms; i++) {
			int x_pos = (int) sketcher.random(_temp_x * i, _temp_x * (i + 1));
			int y_pos = (int) sketcher.random(_temp_y * i, _temp_y * (i + 1));
			Platform temp = new Platform(null, x_pos, y_pos, 60, 10);
		...
		Floor temp = new Floor(null, height, width);
		scene.put(temp.GAME_OBJECT_ID, temp);
	}
	...
	public static void main(String[] args) {
		...
		createScene(scene); 
		new Thread(new ClientConnectionHandler(scene)).start();
	}
